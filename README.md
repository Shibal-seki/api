...
Bonjour, dans ce projet, j'ai développé un client en ligne de commande (CLI) en Bash pour interagir avec l'API REST de MonitoringApp.  
...
Ce token est stocké dans une variable appelée `$TOKEN`.
...
Chaque fonction correspond à un endpoint de l'API. Par exemple :
```bash
...
## 📊 5. Exemples d'actions (1 min 30)

### ✔️ Création d'un groupe
L'utilisateur saisit un nom et une description.  
...
J'ai volontairement simplifié le code pour le rendre lisible, pédagogique et facile à maintenir.
...
- une interface plus jolie (couleurs)
- un stockage du token dans un fichier
- une version PowerShell pour Windows
